...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(1, 73) and callActive then
            ...
            SendNotification("Contestastes la llamada")
...
AddEventHandler("call:callIncoming", function(job, pos)
    ...
    work = job
    target.pos = pos
    SendNotification("Presione ~g~X~s~ para atender la llamada o ~g~N~s~ para rechazarla")
end)

RegisterNetEvent("call:taken")
AddEventHandler("call:taken", function()
    callActive = false
    SendNotification("Hizo la llamada")
end)

RegisterNetEvent("target:call:taken")
AddEventHandler("target:call:taken", function(taken)
    if taken == 1 then
        SendNotification("¡Alguien viene!")
    elseif taken == 0 then
        SendNotification("Nadie vendrá, lo siento ...")
    elseif taken == 2 then
        SendNotification("Por favor intentelo en otro momento")
    end
end)
