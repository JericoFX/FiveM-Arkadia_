...
function robNpc(targetPed)
    ...
    ESX.Game.Utils.DrawText3D(roblocalcoords, "Pulsa E para robarle", 0.25)
    elseif lasttargetPed == targetPed then
    ESX.Game.Utils.DrawText3D(roblocalcoords, "Estas robando actualmente...", 0.25)
    else
        ...
        if IsControlJustReleased(0, 38) then
            ...
            if not currentrobbing then
                if lasttargetPed == targetPed then
                    ...
                    ESX.ShowNotification("Esta persona ya fue robada, ya no tiene nada más.")
                    ...
                    exports['progressBars']:startUI(Config.RobWaitTime * 1000, "Robando...")
                    end 
                    ...
                    if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, tCoords.x, tCoords.y, tCoords.z, true) <= 5.0 then 
                       if not IsPedDeadOrDying(targetPed) then
                        ...
                        ESX.ShowNotification("Estas muy lejos del objetivo")
                        lastrobbed = math.random(1, 100)
                        if lastrobbed <= Config.PoliceNotify then
                            ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
                                local sex = nil
...
AddEventHandler('muggingNotify', function(alert, xPlayer, coords)
        if  ESX.PlayerData.job.name == 'police' then  
            if Config.GCPhone then 
                ...
                ESX.ShowAdvancedNotification('911 Emergency', 'Robando', alert, 'CHAR_CALL911', 1)
            end
        end
end)
