...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presiona ~INPUT_CONTEXT~ para abrir el menu de ~b~gimnasio~w~')
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then
							...
							ESX.ShowNotification("Tu necesitas ~r~60 segundos ~w~antes de realizar otro ejercicio.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Tu necesitas el carnet de socio para poder realizar ~r~ejercicio")
						end
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presiona ~INPUT_CONTEXT~ para hacer algunas ~g~dominadas')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presiona ~INPUT_CONTEXT~ para realizar ~g~ejercicio')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(situps) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presiona ~INPUT_CONTEXT~ para realizar el ~g~ejercicio')
...
function CheckTraining()
	...
	if resting == false then
		ESX.ShowNotification("Ya puedes hacer ejercicio de nuevo...")
	end
end
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_menu',
        {
            ...
            elements = {
				...
				{label = 'Horario negocio', value = 'hours'},
        ...
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Estamos abiertos 24/7. Bienvenido!")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            ...
            elements = {
				{label = 'Batidos de Proteinas ($150)', value = 'protein_shake'},
				{label = 'Barrita energetica ($150)', value = 'sportlunch'},
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        ...
        {
            title    = 'Gimnasio - Membresia',
            elements = {
				{label = 'Carnet miembro ($250)', value = 'membership'},
            }
        },
        function(data, menu)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'bike_menu',
        {
            ...
            elements = {
				...
				{label = 'Cruiser ($120)', value = 'cruiser'},
