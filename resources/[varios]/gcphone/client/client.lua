...
local KeyOpenClose2 = 21 -- shift izq
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(0)
    if IsControlJustPressed(0, KeyOpenClose) and IsControlPressed (0, KeyOpenClose2) then
  ...
  while true do 
    ...
    for i, _ in pairs(PhoneInCall) do 
        ...
        if (dist <= 5.0) then
          ...
          if (dist <= 1.5) then 
            SetTextComponentFormat("STRING")
            AddTextComponentString("~INPUT_PICKUP~ Descolgar Telefono")
