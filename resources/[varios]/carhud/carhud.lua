...
local cruisekey = 20 -- F9
--local seatbeltkey = 311 -- K
local EngineHpBroken = 110
local EngineHpAlmostBroken = 320
	...
	--  ####         	 PLANE HUD	  	  	  ####  --			
			...
			if HUD.Cruise and PedHeli == false and PedPlane == false and PedBoat == false then
				if cruisecolor == false then
					drawTxt(UI.x + 0.535, UI.y + 1.245, 1.0,1.0,0.45, "", 255, 0, 0,200) -- Red 
				else
					drawTxt(UI.x + 0.535, UI.y + 1.245, 1.0,1.0,0.45, "", 0, 255, 0,200) -- Green
				end
			...
			if HUD.Top and PedHeli == false and PedPlane == false and PedBoat == false then
				drawTxt(UI.x + 0.540, 	UI.y + 1.2624, 1.0,1.0,0.55, "~g~Matrícula:~w~" .. PlateVeh, 255, 255, 255, 255)
...
AddEventHandler('pv:setCruiseSpeed', function()
	if cruise == 0 and IsPedInAnyVehicle(GetPlayerPed(-1), false) then
		if GetEntitySpeedVector(GetVehiclePedIsIn(GetPlayerPed(-1), false), true)['y'] > 0 then
			...
			--NotificationMessage("CruiseControl: ~g~ T�NDT~w~ - ~b~ " .. cruiseKm .." km/t")
			...
			Citizen.CreateThread(function()
				while cruise > 0 and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) do
					...
					if cruise > 100 then
						cruise = 0
						NotificationMessage("Limitador de velocidad: No se puede establecer tan alto!")
						break
					end
					Wait(200)
				end
