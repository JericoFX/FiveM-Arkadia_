		...
		if IsControlJustPressed(0, 51) and not onMission and IsPedInAnyVehicle(PlayerPedId()) then
			...
			if IsMissionVehicle(GetEntityModel(veh)) then
				...
				if time >= 0 and time <= TimeToSeconds(5, 30, 0) then
					...
					ShowMPMessage("Ayanamiento", "Busca una~r~Casa ~s~Que robar.", 5000)
					--ShowSubtitle("Find a ~r~house ~s~ to rob")
				else
					DisplayHelpText("Las misiones de ayanamiento solo son de 0:00 a 5:30 AM.")
				end
			end
		end
		...
		if onMission then		
			for k,door in pairs(doors) do
				...
				if Vdist(coords, door.coords.x, door.coords.y, door.coords.z) < 2 then
					DisplayHelpText("Presiona L para entrar en la casa.")
					...
					if IsControlJustPressed(0, 182) then
						...
						ShowSubtitle("Has conseguido entrar, ten cuidado y no hagas ruido.")
					end
				end
			end
		end
