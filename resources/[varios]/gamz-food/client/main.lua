    ...
    while true do
        ...
        for k in pairs(Config.Zones) do
            if GetDistanceBetweenCoords(Config.Zones[k].x, Config.Zones[k].y, Config.Zones[k].z, coords) < 1 then
                Marker("~w~[~r~E~w~] Comprar comida/bebida", 27, Config.Zones[k].x, Config.Zones[k].y, Config.Zones[k].z - 0.99)
                if IsControlJustReleased(0, Keys['E']) then
                    FoodMeny()
                end
            elseif GetDistanceBetweenCoords(Config.Zones[k].x, Config.Zones[k].y, Config.Zones[k].z, coords) < 10 then
                Marker("~w~Comprar comida/bebida", 27, Config.Zones[k].x, Config.Zones[k].y, Config.Zones[k].z - 0.99)
            end
        end
    end
end)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'foodstand',
        {
            ...
            elements = {
                ...
                {label = 'Hamburguesa <span style="color:green"> ' .. Config.EatPrice ..' €</span>',                   prop = 'prop_cs_burger_01',    type = 'food'},
                {label = 'Sandwich <span style="color:green"> ' .. Config.EatPrice ..' €</span>',                 prop = 'prop_sandwich_01',     type = 'food'},
                {label = 'Agua con gas 50cl <span style="color:green"> ' .. Config.DrinkPrice ..' €</span>',   prop = 'prop_ld_flow_bottle',  type = 'drink'},
