    ...
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
        ...
        for key, vehicleData in ipairs(fetchedVehicles) do
            ...
            table.insert(menuElements, {
                ["label"] = "Uso " .. GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " Matricula - " .. vehicleData["plate"],
                ["vehicle"] = vehicleData
            })
        end
            ...
            for _, vehicle in ipairs(gameVehicles) do
                if DoesEntityExist(vehicle) then
                    ...
                    if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                        table.insert(menuElements, {
                            ["label"] = GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " con matricula - " .. vehicleData["plate"] .. " - " .. dstCheck .. " metros.",
...
ChooseVehicleAction = function(vehicleEntity, callback)
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "second_vehicle_menu", {
        ["title"] = "Selecciona una acción - " .. GetVehicleNumberPlateText(vehicleEntity),
        ["align"] = Config.AlignMenu,
        ["elements"] = menuElements
    }, function(menuData, menuHandle)
...
VehicleAction = function(vehicleEntity, action)
    ...
    if action == "change_lock_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("Estás demasiado lejos del vehículo para comprobarlo.") end
        ...
        ESX.ShowNotification(GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleEntity))) .. " con matricula - " .. GetVehicleNumberPlateText(vehicleEntity) .. " esta ahora " .. (vehicleLockState == 1 and "LOCKED" or "UNLOCKED"))
    elseif action == "change_door_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("Estás lejos del vehículo para revisar.") end
...
SpawnLocalVehicle = function(vehicleProps)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		ESX.ShowNotification("Por favor mueva el vehículo.")
...
SpawnVehicle = function(vehicleProps)
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        ESX.ShowNotification("Has sacado tu vehiculo.")
	...
	if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("garage:validateVehicle", function(valid)
			if valid then
				...
				ESX.ShowNotification("Aparcaste tu vehiculo.")
			else
				ESX.ShowNotification("Estas seguro que esto te pertenece?")
			end
