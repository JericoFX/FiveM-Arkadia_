    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_yellowjack', _U('billing'), amount)
        end
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_yellowjack:getFridgeStockItems', function(items)
    ...
    ESX.UI.Menu.Open(
      ...
      {
        title    = _U('yellowjack_fridge_stock'),
        elements = elements
      },
      function(data, menu)
...
function OpenShopMenu(zone)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'yellowjack_shop',
        {
            title    = _U('shop'),
            elements = elements
        },
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustReleased(0,  Keys['F6']) and IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'yellowjack_actions') then
