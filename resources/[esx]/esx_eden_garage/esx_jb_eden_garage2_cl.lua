...
function OpenMenuGarage(garage, KindOfVehicle)
	...
	local elements = {
		{label = "Devolver vehiculo ("..Config.Price.."$)", value = 'return_vehicle'},
	}
...
function ListVehiclesMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		function(data, menu)
			local elem = {}
			table.insert(elem, {label ="Salir con el vehiculo" , value = 'get_vehicle_out'})
			table.insert(elem, {label ="Renombrar el vehiculo" , value = 'rename_vehicle'})
				...
				function(data2, menu2)
					if data2.current.value == "get_vehicle_out" then
                        if (data.current.value.fourrieremecano) then
                            TriggerEvent('esx:showNotification', 'Su vehículo está en el deposito de mecánicos')
                        elseif (data.current.value.state) then
                            menu.close()
                            menu2.close()
                            SpawnVehicle(data.current.value.vehicle, garage, KindOfVehicle)
                        else
                            TriggerEvent('esx:showNotification', 'Su vehiculo ya esta fuera')
                        end
					elseif data2.current.value == "rename_vehicle" then
						AddTextEntry('FMMC_KEY_TIP8', "Nombre del vehiculo deseado")
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'No puede guardar este vehiculo')
					end
				...
				ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'Su vehiculo esta en el garaje')
					else
						TriggerEvent('esx:showNotification', 'No puedo guardar este vehiculo')
					end
				end,vehicleProps, KindOfVehicle)
			end
		else
			TriggerEvent('esx:showNotification', 'Usted no es un conductor del vehiculo')
		end
	else
		TriggerEvent('esx:showNotification', 'Il n\' No hay vehiculos para conducir')
	end
end
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'El remolque ha sido remolcado al garaje')
					else
						TriggerEvent('esx:showNotification', 'No puede guardar este remolque en el garaje.')
					end
				end,trailerProps)
			else
				local vehicleProps  = ESX.Game.GetVehicleProperties(vehicle)
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'El vehiculo ha entrado en el garaje')
					else
						TriggerEvent('esx:showNotification', 'Usted no puede guardar este vehiculo en el lote de incautacion')
					end
				end,vehicleProps)
			end
		else
			TriggerEvent('esx:showNotification', 'usted no es el conductor del vehiculo')
		end
	else
		TriggerEvent('esx:showNotification', 'tu ¡\' no tienes vehiculos para conducir')
	end
end
...
function ReturnVehicleMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		if not table.empty(vehicles) then
			...
			table.insert(elements, {label ="no hay vehiculo para dejar" , value = nil})
		end
		...
		function(data, menu)
			if data.current.value == 'fourrieremecano' then
				ESX.ShowNotification("vaya a la policia o al mecanico para averiguar como recuperar su vehiculo.")
				...
				if not iscaronearth then
					ESX.TriggerServerCallback('eden_garage:checkMoney', function(hasEnoughMoney)
						if hasEnoughMoney then
							...
							ESX.ShowNotification('tu n\'no tienes suficiente d\'dinero')						
						end
					end)
				else
					ESX.ShowNotification("No puedes sacar este vehiculo, Ve a buscarlo!")
				end				
			end
		end,
		function(data, menu)
