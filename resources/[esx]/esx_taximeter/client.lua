...
local fareCost = 0.8 --(1.66 = $100 per minute) Cost per second
local costPerMile = 8.0
...
Citizen.CreateThread(function()
  while true do
    ...
    if meterActive and GetPedInVehicleSeat(veh, -1) == ped then
      ...
      DecorSetFloat(veh, "miles", _miles + round(GetEntitySpeed(veh) * 0.001, 5))
      TriggerEvent('taxi:updatefare', veh)
    end
    ...
    while true do
      ...
      if(IsInTaxi()) then
        if (inTaxi == false) then
          msg = "Presiona G para visualizar el taximetro"
        ...
        if(IsInTaxi() and GetPedInVehicleSeat(veh, -1) == ped) then
          ...
          if IsControlJustReleased(0, 178)  then -- supr
            TriggerEvent('taxi:toggleHire')
            Citizen.Wait(100)
          end
          if IsControlJustReleased(0,121) then -- ins
...
AddEventHandler('vRP_taxi:user_settings', function(action, value)
  if action ~= nil and IsInTaxi() then
    ...
    if GetPedInVehicleSeat(veh, -1) == ped then
      if action == "show" then
        msg = "<b>Valores actuales del medidor</b></b><br /><b>Inicial</b> = "..initialFare.."€<br /><b>Precio por milla</b> = "..costPerMile.."€<br /><b>Precio por minuto</b> = $"..fareCost*60
      elseif action == "initial" then
        initialFare = value*1.0
        DecorSetFloat(veh, "fares", initialFare)
        TriggerEvent('taxi:updatefare', veh)
        msg = "<b>Tarifa inicial establecida en </b>€"..value
        DecorSetFloat(veh, "initialFare", value*1.0)
      elseif action == "mile" then
        costPerMile = value
        msg = "<b>Tarifa por milla establecida en </b>€"..value
        DecorSetFloat(veh, "costPerMile", value)
      elseif action == "minute" then
        fareCost = value/60
        msg = "<b>Tarifa por minuto establecida en </b>€"..value
