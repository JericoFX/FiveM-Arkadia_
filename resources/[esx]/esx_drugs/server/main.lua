...
ESX.RegisterServerCallback('esx_drugs:isCooldown',function(source, cb)
    cb(cooldown)
end)
...
AddEventHandler('esx_drugs:pickedUpCannabis', function()
	...
	if xItem.limit ~= 40 and (xItem.count + 1) > xItem.limit then
...
ESX.RegisterServerCallback('esx_drugs:canPickUp', function(source, cb, item)
	...
	if xItem.limit ~= 80  and xItem.count >= xItem.limit then
		cb(false)
	else
		cb(true)
	end
	...
	if not playersProcessingCannabis[source] then
		...
		playersProcessingCannabis[_source] = ESX.SetTimeout(Config.Delays.WeedProcessing, function()
			...
			elseif xCannabis.count < 1 then
				...
				xPlayer.addInventoryItem('marijuana', 4)
...
RegisterServerEvent('chopNotify2')
AddEventHandler('chopNotify2', function()
